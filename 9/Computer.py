class Computer:
    def __init__(self, name, program):
        ...
        self.program.extend([0]*len(program))
        self.running = True
    ...
    def __readParam(self, mode, pos):
        ...
        if mode[3-pos] == '0':
            ...
            relIndex = self.__relativeBase + param
            outParam = self.program[relIndex]
        else:
            outParam = param
        return outParam

    def __writeParam(self, mode, paramPos, value):
        posIndex = self.__readParam('111', paramPos)
        if mode[3-paramPos] == '2':
            posIndex += self.__relativeBase
    ...
    def __opcode1(self, mode):
        ...
        self.__writeParam(mode, 3, val1 + val2)
    ...
    def __opcode3(self, mode, givenInput = None):
        ...
        self.__writeParam(mode, 1, numIn)
    ...
    def __opcode7(self, mode):
        ...
        if p1 < p2:
            toStore = 1
        self.__writeParam(mode, 3, toStore)
        self.__endInstruction(4)
    ...
    def __opcode9(self, mode):
        self.__relativeBase += self.__readParam(mode, 1)
        self.__endInstruction(2)
